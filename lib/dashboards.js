                ...
                Object.entries(LIMITS.excludeOrg).forEach(([collname, methods]) => {
                    Object.entries(methods).forEach(([method, fixQuery]) => {
                        ...
                        coll[method] = function() {
                            ...
                            if (method == "count" && collname == "pull_request" && limit.limitType == "repo") {
                                console.log(util.inspect(nargs, {depth:null}));
                            }
                            return orig.apply(coll, nargs);
                        }
                    })
...
function assembleDashboard(options) {
    ...
    */
    return new Promise((resolve, reject) => {
        ...
        // special handling for teams, who have a title and inside/outside org
        if (options.limit.limitType == "team") {
            outputSlug = outputSlugTitle;
            writeRedirect = false;
        }
...
function dashboardForEachTeam(options) {
    ...
    var dashboardMakers = Object.keys(options.userConfig.teams).map(teamname => {
