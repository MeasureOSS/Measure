...
const query_cache = require('./query_cache');
                ...
                if (replacements) {
                    Object.entries(replacements).forEach(([method, fixQuery]) => {
                        ...
                        coll[method] = function() {
                            ...
                            return query_cache.execute_with_cache(orig, coll, collname, method, nargs, options);

                            var cache_key = [orig, coll, collname, method, nargs];
                            var cached_result = query_cache.get(cache_key);

                            /* Calling this function either returns a Promise, or it returns
                               a Cursor, or it doesn't return anything and calls its callback.
                               We can test whether it calls its callback by checking if it 
                               HAS a callback, by looking at the last item in nargs.
                               */
                            if (typeof(nargs[nargs.length-1]) == "function") {
                                /* This calls a callback. */

                                var realfn = nargs[nargs.length-1];
                                console.log("This calls a callback", collname, method, mylimit.limitType, mylimit.value);

                                if (cached_result) {
                                    return realfn(null, cached_result);
                                }

                                /* The result isn't in the query cache. So, we need to run the query, and
                                   then cache its results. So, override the callback to steal the
                                   results and put them in the cache. */
                                nargs[nargs.length-1] = function(err, results) {
                                    if (err) { return realfn(err, results); }
                                    query_cache.set(cache_key, results);
                                    //console.log("We called a real function and got results", results.length);
                                    return realfn(err, results);
                                }
                                return orig.apply(coll, nargs);
                            } else {
                                /* This doesn't have a callback, so we're chaining methods and then
                                   returning a promise at the end. */

                                var ret = orig.apply(coll, nargs);
                                console.log("Return type from 'Promise' is", method, typeof(ret.toArray), typeof(ret.then));
                                return ret;
                            }
                        }
                    })
                ...
                Object.entries(LIMITS.excludeOrg).forEach(([collname, methods]) => {
                    Object.entries(methods).forEach(([method, fixQuery]) => {
                        ...
                        coll[method] = function() {
                            ...
                            var cached_result = query_cache.get(nargs);
                            if (cached_result) {
                                return cached_result;
                            } else {
                                var result = orig.apply(coll, nargs);
                                //query_cache.set(nargs, result);
                                return result;
                            }
                        }
                    })
