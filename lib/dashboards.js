...
function assembleDashboard(options) {
    ...
    */
    return new Promise((resolve, reject) => {
        ...
        const outputFile = path.join(options.userConfig.output_directory, outputSlug.toLowerCase());
        const outputFileRedirect = path.join(options.userConfig.output_directory, outputSlugRedirect.toLowerCase());
        ...
        options.templates.dashboard(tmplvars, (err, output) => {
            try {
                ...
                fs.writeFile(outputFile, output, {encoding: "utf8"}, err => {
                    ...
                    if (writeRedirect) {
                        var redirectParams = {
                            outputSlugAll: path.basename(includeExcludeOrgFilenames[0]).toLowerCase(), 
                            outputSlugExcludeOrg: path.basename(includeExcludeOrgFilenames[1]).toLowerCase()
                        };
...
function dashboardForEachRepo(options) {
    ...
    if (options.userConfig.debug) { console.log(`Assembling a dashboard for each repository (${dashboardMakers.length} of them) @ ${moment()}`); }
...
function dashboardForEachOrg(options) {
    if (options.userConfig.debug) { console.log(`Assembling a dashboard for each org (${Object.keys(options.org2People).length} of them) @ ${moment()}`); }
    return new Promise((resolve, reject) => {
        async.map(Object.keys(options.org2People), (orgName, done) => {
            if (options.org2People[orgName].length === 0) {
                // this org does not and never did have any people in it.
                // don't generate a dashboard for it.
                console.log("Warning: not generating a dashboard for " +
                    "organization " + orgName + " since it has never had " +
                    "any people assigned to it.");
...
function dashboardForEachTeam(options) {
    ...
    var dashboardMakers = dashboardMakersAll.concat(dashboardMakersExcludeOrg);
    if (options.userConfig.debug) { console.log(`Assembling a dashboard for each team (${dashboardMakers.length} of them) @ ${moment()}`); }
    return Promise.all(dashboardMakers)
        .then(function(arrayOfOptions) {
            var optionsBase = Object.assign({}, arrayOfOptions[0]);
            delete optionsBase.repo;
...
function getUsers(options) {
    return new Promise((resolve, reject) => {
        options.db.collection("user").find({}, {login:1}).toArray().then(users => {
            ...
            if (options.userConfig.skip_contributor_dashboards) {
                skipList = skipList.concat(userList);
                userList = [];
            }
...
function dashboardForEachContributor(options) {
    ...
    actually fail.
    */
    if (options.userConfig.debug) { console.log(`Assembling a dashboard for each contributor @ ${moment()}`); }
    return new Promise((resolve, reject) => {
...
function changedContributors(options) {
    /* Generating contributor dashboards is the slowest part of the process,
       ...
       to regenerate them). */
    if (options.userConfig.debug) { console.log("Calculating list of contributors"); }
    return new Promise((resolve, reject) => {
