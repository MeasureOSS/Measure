        ...
        var ipgs = [
            ...
            },
            {
                ...
                props: {
                    ...
                    }).filter(org => { return org.count_current + org.count_left > 0; }),
            ...
            try {
                ...
                fs.writeFile(outputFile, output, {encoding: "utf8"}, err => {
                    try {
                        ...
                        options.templates.redirect({outputSlugAll:path.basename(outputSlugAll).toLowerCase(), outputSlugExcludeOrg:path.basename(outputSlugExcludeOrg).toLowerCase()}, (err, output) => {
            ...
            try {
                ...
                for (var orgname in options.org2People) {
                    options.org2People[orgname].forEach(p => {
                        if (!people2Org[p.login]) people2Org[p.login] = [];
                        people2Org[p.login].push(orgname);
                    });
                }

                // get contributor full names
                options.db.collection("user").find({login: {$in: Object.keys(all_contributors)}}, {name:1, login:1, company:1, location:1, email:1}).toArray().then(users => {
                    try {
                        users.forEach(u => {
                            if (all_contributors[u.login]) {
                                all_contributors[u.login].name = u.name;
                                all_contributors[u.login].company = u.company;
                                all_contributors[u.login].location = u.location;
                                all_contributors[u.login].email = u.email;
                            }
                        })
                        ...
                        for (var k in all_contributors) {
                            searchdata.push({
                                ...
                                company: all_contributors[k].company,
                                location: all_contributors[k].location,
                                email: all_contributors[k].email,
                                login: all_contributors[k].login,
                                org: people2Org[all_contributors[k].login] || [],
