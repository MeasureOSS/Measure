...
function loadTemplates() {
    ...
    */
    const TEMPLATES_LIST = ["list", "bignumber", "graph", "dashboard", "front", 
        ...
        "organizations", "report", "reportlist"];
...
function writeReport(result, options) {
    return new Promise((resolve, reject) => {
        ...
        var tmplvars = Object.assign(result.result, {isReport: true});
...
function writeReportList(reports, options) {
    return new Promise((resolve, reject) => {
        ...
        options.templates.reportlist({reports:reports, isReport: true}, (err, output) => {
            if (err) return reject(err);
            output = fixOutputLinks(output, outputFile, options);
            fs.writeFile(outputFile, output, {encoding: "utf8"}, err => {
                if (err) {
